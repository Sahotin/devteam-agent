...
Python Runner 预装固定版本的 pytest 与 Robot Framework。Robot 执行结果会写入项目的
`.devteam/test-results/robot/`，供 Tester Agent 解析为结构化用例统计。被测项目的额外依赖仍需
通过后续的项目专用 Runner 镜像机制提供。Node 和 Maven Runner 当前要求依赖已经存在于项目
Workspace 或预构建镜像中。
