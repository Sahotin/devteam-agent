...
def test_browser_reproduction_overrides_inconclusive_model_diagnosis(
    client: TestClient,
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    source_task_id = move_task_to_testing(client, tmp_path / "interaction-bug")
    assert client.post(f"/api/v1/tasks/{source_task_id}/test").json()["state"] == "COMPLETED"
    iteration = client.post(
        f"/api/v1/tasks/{source_task_id}/iterations",
        json={
            "kind": "BUG_FIX",
            "request": "点击“开始游戏”按钮之后没有反应",
        },
    ).json()
    monkeypatch.setattr(
        VisualReviewerAgent,
        ...
        classmethod(
            lambda _cls, root, task_id, report: InteractionProbeResult(
                attempted=True,
                target="开始游戏",
                found=True,
                changed=False,
    ...
    response = client.post(f"/api/v1/tasks/{iteration['id']}/diagnosis")

    assert response.status_code == 200, response.text
    diagnosis = response.json()["content"]
    assert diagnosis["status"] == "CONFIRMED"
    assert diagnosis["requires_code_change"] is True
    assert "setLoadingState" in diagnosis["root_cause"]
