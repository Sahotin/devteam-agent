...
def capabilities(container: Container) -> CapabilityResponse:
    ...
    return CapabilityResponse(
        ...
        model_profiles=[
            ...
            for profile in container.model_router.profiles
        ],
        agent_model_tiers={
        ...
        governance_model_tiers={
            governance.value: {
                agent_name: tier.value
                for agent_name, tier in assignments.items()
            }
            for governance, assignments in (
                container.model_router.governance_assignments.items()
            )
        },
...
@router.post(
    ...
    response_model=WorkspaceAccessReport,
    tags=["projects"],
)
def check_project_workspace(
    project_id: str, container: Container
) -> WorkspaceAccessReport:
    project = container.repository.get_project(project_id)
    try:
        ensure_workspace_writable(project.root_path)
    except WorkspaceWritePermissionError as error:
        return WorkspaceAccessReport(
...
def update_project_workspace(
    project_id: str,
    command: ProjectWorkspaceUpdate,
    container: Container,
) -> ProjectRecord:
    ensure_workspace_writable(command.root_path)
    return container.repository.update_project_workspace(project_id, command)
...
@router.get(
    ...
    response_model=TaskEvaluationReport,
    tags=["evaluation"],
)
def get_task_evaluation(
    task_id: str, container: Container
) -> TaskEvaluationReport:
    return container.evaluation_service.get_task_report(task_id)


@router.post(
    "/tasks/{task_id}/evaluation-feedback",
    response_model=TaskEvaluationFeedbackRecord,
    tags=["evaluation"],
)
def submit_task_evaluation_feedback(
    task_id: str,
    command: TaskEvaluationFeedbackCreate,
    container: Container,
) -> TaskEvaluationFeedbackRecord:
    return container.evaluation_service.submit_feedback(task_id, command)


@router.get(
    "/projects/{project_id}/evaluation-summary",
    response_model=ProjectEvaluationSummary,
    tags=["evaluation"],
)
def get_project_evaluation_summary(
    project_id: str, container: Container
) -> ProjectEvaluationSummary:
    return container.evaluation_service.get_project_summary(project_id)


@router.get(
    "/evaluation/benchmark",
    response_model=PolicyBenchmarkReport,
    tags=["evaluation"],
)
...
def revise_prd(
    task_id: str,
    command: PrdRevisionRequest,
    container: Container,
) -> ArtifactRecord:
    return container.workflow.revise_prd(
        task_id,
        command.content,
...
async def decide_architecture(
    ...
    return await container.workflow.decide_architecture(
        ...
        selected_design_option_id=command.selected_design_option_id,
...
@router.get(
    ...
    response_model=ProjectRuntime,
    tags=["delivery"],
)
def get_project_runtime(task_id: str, container: Container) -> ProjectRuntime:
    return container.runtime_manager.get(task_id)


@router.get(
    "/tasks/{task_id}/visual-screenshots/{screenshot_path:path}",
    tags=["delivery"],
)
def get_visual_screenshot(
    task_id: str,
    screenshot_path: str,
    container: Container,
) -> FileResponse:
    task = container.repository.get_task(task_id)
    project = container.repository.get_project(task.project_id)
    artifact = container.repository.latest_artifact(
        task_id, ArtifactType.VISUAL_REPORT
    )
    allowed = {
        str(item)
        for item in artifact.content.get("screenshots", [])
    }
    if screenshot_path not in allowed:
        raise HTTPException(status_code=404, detail="截图不存在或不属于当前任务")
    root = Path(project.root_path).expanduser().resolve()
    target = (root / screenshot_path).resolve()
    try:
        target.relative_to(root)
    except ValueError as error:
        raise HTTPException(status_code=400, detail="截图路径不安全") from error
...
@router.post(
    "/tasks/{task_id}/runtime/start",
    response_model=ProjectRuntime,
    tags=["delivery"],
)
def start_project_runtime(task_id: str, container: Container) -> ProjectRuntime:
    return container.runtime_manager.start(task_id)


@router.post(
    "/tasks/{task_id}/runtime/install-dependencies",
    response_model=ProjectRuntime,
    tags=["delivery"],
)
def install_project_runtime_dependencies(
    task_id: str,
    command: DependencyInstallRequest,
    container: Container,
) -> ProjectRuntime:
    return container.runtime_manager.install_dependencies(
        task_id,
...
@router.post(
    "/tasks/{task_id}/runtime/stop",
    response_model=ProjectRuntime,
    tags=["delivery"],
)
def stop_project_runtime(task_id: str, container: Container) -> ProjectRuntime:
    return container.runtime_manager.stop(task_id)
...
def list_events(
    ...
    before_event_id: Annotated[int | None, Query(ge=1)] = None,
) -> list[EventRecord]:
    container.repository.get_task(task_id)
    return container.repository.list_recent_events(
        task_id,
        limit=limit,
...
async def stream_events(
    ...
    async def generate_events():
        cursor = max(after_event_id, last_event_id or 0)
        idle_ticks = 0
        while True:
...
def list_tool_calls(
    ...
    container.repository.get_task(task_id)
    return container.repository.list_recent_tool_calls(task_id, limit=limit)
...
async def cancel_execution(
    ...
    return await container.execution_manager.cancel(execution_id)
...
def task_observability(
    ...
    task = container.repository.get_task(task_id)
    events = container.repository.list_events(task_id)
    governance = (
    ...
    return TaskObservabilityResponse(
        ...
        tool_calls=container.repository.list_recent_tool_calls(task_id, limit=100),
