...
class SqlAlchemyRepository:
    ...
    def create_task(self, command: TaskCreate) -> TaskRecord:
        with self._session() as session:
            if session.get(ProjectRow, command.project_id) is None:
    ...
    def create_iteration_task(
        ...
        with self._session() as session:
            ...
            if TaskState(source.state) is not TaskState.COMPLETED:
                raise ValueError("only a completed task can create an iteration")
            row = TaskRow(
                id=str(uuid4()),
            ...
            session.add_all(
                [
                    EventRow(
                        task_id=row.id,
                        event_type="task.created",
                        payload={"state": row.state, "source": "iteration"},
                    ),
                    EventRow(
                        task_id=row.id,
