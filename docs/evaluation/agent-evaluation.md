...
2026-08-24 使用 DeepSeek Provider、DYNAMIC 策略和 STANDARD-001 场景执行过一次成本探针。该轮不是成功率实验，结果不能作为模型能力结论：

- 工作流未完成，质量门禁未通过；
- 共记录 7 次模型路由尝试、312,978 Token；
- 模型调用累计耗时 505,239 ms，墙钟耗时 509,054 ms；
- 最终失败源于开发返工计划把 REV-* 审查问题编号误当成 PRD 需求编号；
- 同一强档被重复加入升级路线，造成一次没有能力增益的额外等待。

该探针直接推动了两项代码修复：最高档位路由去重，以及依据审查产物中的 requirement_ids 将返工问题编号确定性映射回 FR/NFR。修复已加入自动化回归测试；尚未执行同场景三轮复测，因此不得在简历中表述为“成功率提升”或“Token 降低”。原始报告位于本地忽略目录 reports/generated/，默认不提交可能包含业务上下文的实验文件。
