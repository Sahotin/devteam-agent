...
export function IterationModal({ task, busy, onClose, onSubmit, failureOnly = false }: IterationModalProps) {
  const [kind, setKind] = useState<IterationKind>("BUG_FIX");
  const [request, setRequest] = useState("");
  const availableOptions = failureOnly
    ? iterationOptions.filter((item) => item.kind === "BUG_FIX")
    : iterationOptions;
  const selected = availableOptions.find((item) => item.kind === kind)!;
    ...
    <div className="modal-backdrop" role="presentation" onMouseDown={busy ? undefined : onClose}>
      ...
      >
        <div className="modal-heading">
          <div>
            <span className="eyebrow">{failureOnly ? "故障升级处理" : "继续迭代"}</span>
            <h2>{failureOnly ? "提交失败信息并创建修复任务" : "告诉智能体接下来要修改什么"}</h2>
          </div>
        ...
        <div className="iteration-context">
          <span>{failureOnly ? "基于执行失败的任务" : "基于已完成任务"}</span>
        ...
        <div className="iteration-actions">
          ...
          <button type="submit" className="primary-button" disabled={busy || request.trim().length < 5}>
            {busy ? "正在创建迭代…" : failureOnly ? "创建并启动修复任务" : "创建并启动迭代"}
          </button>
        </div>
      </form>
    </div>
